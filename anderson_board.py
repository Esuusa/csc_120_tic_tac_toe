...
def main():
  ...
  for turn in range(9):
    ...
    # checks  for win if three or more turns have been made
    if turn > 1:
      player_win = check_win(player_id)
      if player_win == True:
        print('\nPlayer ' + str(player_id) + ' has won! Congratulations!')
        print_board()
        return
  # Prints a 'no one wins' message if all spaces are occupied and no one won
  print('\nNo one wins. Cat\'s game!')
  print_board()
